...
class CustomUserModelStrTest(TestCase):
    def test_custom_user_str(self):
        user = CustomUser.objects.create_user(username="testuser", email="testuser@nyu.edu")
...
class RentalListingsModelTests(TestCase):
    def setUp(self):
        self.user = CustomUser.objects.create_user(username='testuser', email='test@nyu.edu', password='testpass123')
...
class FavoriteModelTests(TestCase):
    def setUp(self):
        self.user = CustomUser.objects.create_user(username='favuser', email='favuser@nyu.edu', password='testpass123')
        self.listing = Rental_Listings.objects.create(address='123 Fav St', price=1500.00, baths=1, Landlord=self.user)
...
class UserLoginViewTests(TestCase):
    def test_user_login_redirect(self):
        user = CustomUser.objects.create_user(username="loginuser", email="loginuser@nyu.edu", password="password")
        response = self.client.post(reverse("user_login"), {"username": "loginuser", "password": "password"})
        self.assertRedirects(response, reverse("user_homepage"))
...
class UserHomePageAuthenticatedAccessTests(TestCase):
    def test_user_homepage_access_authenticated(self):
        user = CustomUser.objects.create_user(username="testuserhome", email="testuserhome@nyu.edu", password="password")
...
class LandlordSignupWithFileTest(TestCase):
    ...
    def test_landlord_signup_with_file(self, mock_boto3_client):
        ...
        form_data = {
            ...
            'phone_number': '9876543210',
            'city': 'New York',
            'pdf_file': SimpleUploadedFile('test_file.pdf', b'These are the file contents.', content_type='application/pdf')
        }
        response = self.client.post(reverse('landlord_signup'), form_data)
        self.assertRedirects(response, reverse('landlord_login'))
        self.assertTrue(CustomUser.objects.filter(email='landlordfile@nyu.edu').exists())


class UserLoginRedirectTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.user = CustomUser.objects.create_user(username='testuserredirect', email='userredirect@nyu.edu', password='password123', user_type=CustomUser.USER)
...
class Custom404HandlerTest(TestCase):
    ...
    def test_404_redirect_for_authenticated_user(self):
        user = CustomUser.objects.create_user(username='user404test', password='password')
...
class ToggleFavoriteTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.user = CustomUser.objects.create_user(username='favuser', email='favuser@nyu.edu', password='testpass123', user_type=CustomUser.USER)
        cls.listing = Rental_Listings.objects.create(address='123 Fav St', price=1500.00, baths=1, Landlord=cls.user)
...
class LandlordUserPageAccessTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.landlord_user = CustomUser.objects.create_user(username='landlorduser', email='landlorduser@nyu.edu', password='testpass123', user_type=CustomUser.LANDLORD)
        cls.user = CustomUser.objects.create_user(username='normaluser', email='normaluser@nyu.edu', password='testpass123', user_type=CustomUser.USER)
